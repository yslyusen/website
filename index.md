...
## Research
What I have done so far is following Professor Swift's advice and learning every detail of Linux device drivers implementation. This is hard for those who have never stepped into the kernel but only called several syscalls in their C programs, with limited knowledge in computer architecture. For instance, take a look at the implementation of the most widely used function [```malloc()```](https://elixir.bootlin.com/linux/latest/source/include/linux/decompress/mm.h#L34). Hence, I spent lots of time going through the book Linux Device Drivers, but I do believe I have only grabbed the overview of the device drivers. In order to deal with so much details, I did something more, which is shown below.

When I was going through the example codes, I found the [code](https://github.com/martinezjavier/ldd3) that supports the latest kernel version. I made my own [repository](https://github.com/yslys/ldd) and added detailed comments in it so that it will help me recall each step of the code; furthermore, for those who are interested in Linux Device Drivers, such repository could as well be one of the references.
